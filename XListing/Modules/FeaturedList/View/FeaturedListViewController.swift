...
//        setupDisplayingFeaturedList()
        ...
        let businessVMSignal = dynamicArraySignal?.map { (changedValues, change, indexSet) -> BusinessViewModel in
            if changedValues?.count > 1 {
                //MARK: maybe shouldn't be more than one item in the array???
        ...
        nameSChineseLabelSignal = businessVMSignal?.map { businessVM -> NSString? in
            return businessVM.nameSChinese
        }
        
        nameEnglishLabelSignal = businessVMSignal?.map { businessVM -> NSString? in
            return businessVM.nameEnglish
        }
        
        
        addressLabelSignal = businessVMSignal?.map { businessVM -> NSString? in
            return businessVM.address
        }
        
        resultSignal = businessVMSignal?.map { businessVM -> NSString? in
